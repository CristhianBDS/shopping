<?php
// admin/pedidos.php — Listado de pedidos (PDO) + filtros + paginación
...
// Tu schema real tiene: order_items(id, order_id, product_id, name, price, qty, subtotal)
$qtyCol   = pickColumn($pdo, 'order_items', ['qty','quantity','cantidad','cant','qty_ordered']) ?? 'qty';
$priceCol = pickColumn($pdo, 'order_items', ['price','unit_price','precio','unitprice'])      ?? 'price';
  ...
  <form method="get" class="row g-2 align-items-end mb-3">
    <div class="col-sm-4 col-md-3">
      ...
      <select name="status" id="status" class="form-select">
        ...
        <?php foreach (['pendiente','confirmado','enviado','cancelado'] as $e): ?>
    ...
    <?php if ($status!=='' || $q!==''): ?>
      <div class="col-auto">
        <a href="<?= $BASE ?>/admin/pedidos.php" class="btn btn-outline-secondary">Limpiar</a>
      </div>
    <?php endif; ?>
  </form>
          ...
          <tbody>
            ...
            <?php else: foreach ($rows as $r): ?>
              <tr>
                ...
                <td class="text-end">
                  <a class="btn btn-sm btn-outline-primary" href="<?= $BASE ?>/admin/pedido.php?id=<?= (int)$r['id'] ?>">Ver</a>
                </td>
              </tr>
